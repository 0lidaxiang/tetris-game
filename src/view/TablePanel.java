...
//	}
	...
	public void paint(Graphics g) {
		...
		for(int i = 0; i < 13; i++){
			for (int j = 0; j < 23; j++ ) {
				if(this.getFlagOfPosition(i, j ,0) == 1){
	...
	public void drawPostionInTable(Graphics g, int colorCode, int x, int y) {
		...
		g.fillRect(x + dimension - shadeWidth, y, shadeWidth, dimension);// right shade
		g.fillRect(x, y + dimension - shadeWidth, dimension, shadeWidth);// bottom shade
	...
	public int getFlagOfPosition(int positionX, int positionY, int flagZ) {
		return positioinFlag[positionX][positionY][flagZ];
	}
}
